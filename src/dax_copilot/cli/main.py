...
[bold yellow]📌 Comandos disponíveis:[/]
  ...
  [bold green]/schema <caminho1> [caminho2...][/]  → Carrega arquivo(s) CSV/Excel como esquema de dados
    ...
    while True:
        try:
            ...
            if user_input.lower().startswith("/schema"):
                ...
                if len(parts) < 2:
                    console.print(
                        "[yellow]⚠️  Uso: /schema <caminho_do_arquivo1> [<caminho2>...][/]\n"
                        "[dim]Exemplo: /schema data/f_vendas.csv data/d_produtos.csv[/]"
                    )
                    continue

                file_paths = [p.strip() for p in parts[1:]]
                try:
                    with console.status(
                        f"[cyan]📂 Lendo esquema de {len(file_paths)} arquivo(s)...[/]"
                    ):
                        schema_text = parse_schema(file_paths)
